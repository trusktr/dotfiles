...
export class SceneManager extends React.Component<SceneManagerProps, SceneManagerState> {
	...
	initTransformControls(): void {
		...
		this.transformControls.addEventListener('change', this.state.annotationManager!.cancelHideTransform)

		// If we just clicked on a transform object don't hide it.
		this.transformControls.addEventListener('mouseDown', this.state.annotationManager!.cancelHideTransform)

		// If we are done interacting with a transform object start hiding process.
		this.transformControls.addEventListener('mouseUp', this.state.annotationManager!.delayHideTransform)

		// If the object attached to the transform object has changed, do something.
		this.transformControls.addEventListener('objectChange', this.state.annotationManager!.updateActiveAnnotationMesh)
	}

...
async function findDefinitionsInHTML(fromPart, currentService, document, cssServiceMap, configuration) {
    ...
    if (fromPart.type === languages_1.PartType.ReactDefaultImportedCSSModuleClass) {
        let filePaths = await languages_1.ModuleResolver.resolveReactDefaultCSSModulePaths(document);
    ...
    if (fromPart.type === languages_1.PartType.ReactImportedCSSModuleProperty) {
        ...
        let filePath = await languages_1.ModuleResolver.resolveReactCSSModuleByName(importedCSSModulePart.text, document);
        if (!filePath) {
            return null;
        }
        let cssModuleService = await cssServiceMap.forceGetServiceByFilePath(filePath);
...
async function findEmbeddedOrImported(currentService, matchPart, fromPart, document, cssServiceMap) {
    let locations = [];

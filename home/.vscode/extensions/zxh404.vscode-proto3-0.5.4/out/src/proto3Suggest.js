...
class Proto3CompletionItemProvider {
    provideCompletionItems(document, position, token) {
        return new Promise((resolve, reject) => {
            ...
            let scope = proto3ScopeGuesser_1.guessScope(document, position.line);
            //console.log(scope.syntax);
            //console.log(textBeforeCursor);
            switch (scope.kind) {
                case proto3ScopeGuesser_1.Proto3ScopeKind.Proto: {
                ...
                case proto3ScopeGuesser_1.Proto3ScopeKind.Message: {
                    if (textBeforeCursor.match(/^\s*\w*$/)) {
                        suggestions.push(kwOption);
                        suggestions.push(kwMessage);
                        suggestions.push(kwEnum);
                ...
                case proto3ScopeGuesser_1.Proto3ScopeKind.Enum: {
                    if (textBeforeCursor.match(/^\s*\w*$/)) {
                        suggestions.push(kwOption);
                    }
                    else if (textBeforeCursor.match(/^\s*option\s+\w*$/)) {

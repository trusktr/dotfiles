...
function activateLanguageServer(context, status, config) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (transportLayer == "tcp") {
            ...
            logger_1.LOG.info(`Connecting via TCP, port: ${tcpPort}`);
        }
        else if (transportLayer == "stdio") {
            logger_1.LOG.info("Connecting via Stdio.");
        }
        else {
            logger_1.LOG.info(`Unknown transport layer: ${transportLayer}`);
        }
...
function createLanguageClient(options) {
    ...
    if (options.tcpPort) {
        ...
        logger_1.LOG.info("Creating client at {}", options.startScriptPath);
    }
...
function spawnLanguageServerProcessAndConnectViaTcp(options) {
    return new Promise((resolve, reject) => {
        logger_1.LOG.info("Creating server.");
        const server = net.createServer(socket => {
            logger_1.LOG.info("Closing server since client has connected.");
            server.close();
            resolve({ reader: socket, writer: socket });
        });
        // Wait for the first client to connect
        server.listen(options.tcpPort, () => {
            const tcpPort = server.address().port.toString();
            const proc = child_process.spawn(options.startScriptPath, ["--tcpClientPort", tcpPort]);
            logger_1.LOG.info("Creating client at {} via TCP port {}", options.startScriptPath, tcpPort);
...
exports.spawnLanguageServerProcessAndConnectViaTcp = spawnLanguageServerProcessAndConnectViaTcp;

...
async function onInitialize(params) {
    ...
    if (options.languageFeatures) {
        let projects;
        const lsConfigs = (0, configs_1.createLsConfigs)(connection);
        const ts = loadTypescript(options.typescript.serverPath);
        (await Promise.resolve().then(() => require('./features/customFeatures'))).register(connection, documents, () => projects);
        (await Promise.resolve().then(() => require('./features/languageFeatures'))).register(ts, connection, documents, () => projects, options.languageFeatures, lsConfigs, params);
        ...
        connection.onInitialized(async () => {
            var _a, _b;
            const inferredCompilerOptions = await (0, inferredCompilerOptions_1.getInferredCompilerOptions)(ts, connection);
            ...
            projects = (0, projects_1.createProjects)(options, ts, tsLocalized, connection, documents, folders, inferredCompilerOptions, lsConfigs);
        });
    }
    return result;
}

...
function formatScalar(value, float) {
    return float ? sprintf_js_1.sprintf('%.5f', value) : `${value}`;
}
...
function getVerticesNode(fileName, gltf, attributes) {
    ...
    for (const attribute in attributes) {
        ...
        const data = accessor && utilities_1.getAccessorData(fileName, gltf, accessor);
        if (!data) {
            continue;
        }
    ...
    for (let index = 0; index < nodes.length; index++) {
        ...
        for (const attribute in attributes) {
            ...
            const values = utilities_1.getAccessorElement(info.data, index, info.numComponents, info.accessor.componentType, info.accessor.normalized);
            attributeNodes.push({
...
class GltfInspectData {
    ...
    showAccessor(fileName, gltf, jsonPointer) {
        ...
        const accessor = utilities_1.getFromJsonPointer(gltf, jsonPointer);
        this._headerNode = {
    ...
    showMeshPrimitive(fileName, gltf, jsonPointer) {
        ...
        this._fileName = fileName;
        this._jsonPointer = jsonPointer;
        const meshPrimitive = utilities_1.getFromJsonPointer(gltf, jsonPointer);
        this._headerNode = {
            type: NodeType.Header,
            label: this._jsonPointer
        };

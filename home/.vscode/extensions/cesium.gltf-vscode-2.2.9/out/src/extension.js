...
function pointerContains(pointer, selection) {
...
function tryGetJsonMap() {
    try {
        return utilities_1.parseJsonMap(vscode.window.activeTextEditor.document.getText());
    }
    catch (ex) {
...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gltf.inspectData', async () => {
        ...
        if (isAccessor) {
            jsonPointer = utilities_1.truncateJsonPointer(jsonPointer, 2);
            gltfWindow.inspectData.showAccessor(fileName, map.data, jsonPointer);
            return;
        }
        if (isMeshPrimitive) {
            jsonPointer = utilities_1.truncateJsonPointer(jsonPointer, 4);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gltf.importUri', () => {
        ...
        const data = utilities_1.getFromJsonPointer(map.data, bestKey);
        ...
        if (dataUri.startsWith('data:')) {
            ...
            dataUri = 'data:' + utilities_1.guessMimeType(name) + ';base64,' + utilities_1.btoa(contents);
            const pointer = map.pointers[bestKey + '/uri'];
            activeTextEditor.edit(editBuilder => {
                editBuilder.replace(new vscode.Range(pointer.value.line, pointer.value.column + 1, pointer.valueEnd.line, pointer.valueEnd.column - 1), dataUri);
            });
        }
    }));
    //
    ...
    //
    function exportToFile(filename, pathFilename, pointer, dataUri) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gltf.exportUri', async () => {
        ...
        if (!dataUri.startsWith('data:')) {
            ...
            if (mimeTypePos > 0) {
                ...
                extension = utilities_1.guessFileExtension(mimeType);
                guessName += extension;
            }
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('gltf.exportGlbFile', async (te, t) => {
        ...
        if (!vscode.workspace.getConfiguration('glTF').get('alwaysOverwriteDefaultFilename')) {
            ...
            if (uri !== undefined) {
                try {
                    gltf_import_export_1.ConvertToGLB(gltf, editor.document.uri.fsPath, uri.fsPath);
            ...
            try {
                gltf_import_export_1.ConvertToGLB(gltf, editor.document.uri.fsPath, glbPath);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('gltf.importGlbFile', async (fileUri) => {
        ...
        try {
            ...
            let targetFilename = await gltf_import_export_1.ConvertGLBtoGltfLoadFirst(fileUri.fsPath, getTargetFilename);
            if (targetFilename) {
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('gltf.importAnimation', async (te, t) => {
        ...
        for (const key of ['input', 'output']) {
            ...
            if (accessor !== undefined) {
                data = utilities_1.getAccessorData(activeTextEditor.document.fileName, glTF, accessor);
            }
    ...
    context.subscriptions.push(vscode.commands.registerTextEditorCommand('gltf.exportAnimation', async (te, t) => {
        ...
        const bufferData = gltf_import_export_1.getBuffer(glTF, bufferIndex, activeTextEditor.document.fileName);

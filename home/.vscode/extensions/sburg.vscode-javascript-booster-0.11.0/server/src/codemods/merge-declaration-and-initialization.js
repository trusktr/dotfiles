...
const codeMod = ((fileInfo, api, options) => {
    ...
    const blockStatementPath = declarationPath.parent;
    ...
    blockStatementPath.node.body.forEach(d => {
        if (j.ExpressionStatement.check(d) &&
            j.AssignmentExpression.check(d.expression) &&
            j.Identifier.check(d.expression.left)) {
            const name = d.expression.left.name;
            const matchedDeclarator = declarationNode.declarations.find(dtor => (j.Identifier.check(dtor) && dtor.name === name) ||
            ...
            declarationNode.declarations.splice(declarationNode.declarations.indexOf(matchedDeclarator), 1);
            ...
            blockStatementPath.node.body.splice(blockStatementPath.node.body.indexOf(d), 1, newDeclaration);
        }
    });
...
codeMod.canRun = (fileInfo, api, options) => {
    ...
    const hasAssignmentsInScope = path.parent.node.body.some(d => j.ExpressionStatement.check(d) &&
        j.AssignmentExpression.check(d.expression) &&
        j.Identifier.check(d.expression.left) &&
...
//# sourceMappingURL=merge-declaration-and-initialization.js.map
...
function isExpressionStatement(j, s) {
    return j.match(s, {
...
function isAssignmentStatement(j, s) {
    return j.match(s, {
        type: 'ExpressionStatement',
        expression: {
...
function isVariableDeclaration(j, s) {
    return (j.match(s, {
    ...
    }) && s.firstNode().declarations.length === 1);
}
const codeMod = ((fileInfo, api, options) => {
    ...
    const parentBlock = s1.parents()[0];
    ...
    statements.forEach(s => {
        ...
        if (isExpressionStatement(j, s)) {
            ...
            dataList.push({
                ...
                awaitArgument: declarator.init.argument
            });
        }
            ...
            dataList.push({
                ...
                awaitArgument: assignmentExpr.right.argument
            });
        }
        else {
    ...
    // Generating new code
    const awaitExpr = j.awaitExpression(j.callExpression(j.memberExpression(j.identifier('Promise'), j.identifier('all')), [
...
function getContainingStatement(j, c) {

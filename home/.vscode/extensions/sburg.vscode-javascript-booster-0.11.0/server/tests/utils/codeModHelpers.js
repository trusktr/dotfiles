...
function extractPosition(modId, source) {
    function extractPosInternal(posKey) {
        const re = /\/\*#\s*([^#]+?)\s*#\*\//g;
        let match;
    ...
    const fullFixtures = fixtures.map(fx => {
        ...
        return {
            ...
            pos: posInfo
                ? posInfo.pos
                : {
                    anchor: new Position_1.Position(1, 1),
                    active: new Position_1.Position(1, 1)
                }
        };
    });
...
function defineCanRunTests(dirName, modId, fixtureId = null, options = {}) {
    ...
    describe(`${modId} can run`, () => {
        inputFixtures.forEach(fx => {
            ...
            const testName = fx.name
                ? `"${modId}:${fx.name}" ${expected ? 'can' : 'cannot'} run (${posToString(fx.pos)})`

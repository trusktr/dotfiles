...
function normalizeColors(colorsNode, text) {
    ...
    colorsNode.forEach(color => {
        ...
        if (color.nodeName === 'ident' && colors_1.colors[color.name]) {
            try {
                const colorObj = colors_1.colorFromHex(colors_1.colors[color.name]);
                const pos = `${color.lineno - 1}-${exports.getRealColumn(color.name, text, color.lineno - 1)}`;
                if (colorPosSet.has(pos)) {
                    // do nothing
                }
                else {
                    colorPosSet.add(pos);
                    colorsInformation.push(new vscode_1.ColorInformation(new vscode_1.Range(new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.name, text, color.lineno - 1)), new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.name, text, color.lineno - 1) + ((_a = color.name) === null || _a === void 0 ? void 0 : _a.length) || 0)), new vscode_1.Color(colorObj.red, colorObj.green, colorObj.blue, colorObj.alpha)));
                }
            }
            catch (_) {
                // do nothing
            ...
            try {
                const pos = `${color.lineno - 1}-${exports.getRealColumn(color.raw, text, color.lineno - 1)}`;
                if (colorPosSet.has(pos)) {
                    // do nothing
                }
                else {
                    colorPosSet.add(pos);
                    colorsInformation.push(new vscode_1.ColorInformation(new vscode_1.Range(new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.raw, text, color.lineno - 1)), new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.raw, text, color.lineno - 1) + ((_b = color.raw) === null || _b === void 0 ? void 0 : _b.length) || 0)), new vscode_1.Color(
                    // @ts-ignore
                    colors_1.getNumericValue(color.r, 255.0), 
                    // @ts-ignore
                    colors_1.getNumericValue(color.g, 255.0), 
                    // @ts-ignore
                    colors_1.getNumericValue(color.b, 255.0), 1)));
                }
            }
            catch (_) {
                // do nothing
            ...
            try {
                ...
                const alpha = colorValues.length === 4 ? colors_1.getNumericValue(colorValues[3], 1) : 1;
                ...
                if (colorPosSet.has(pos)) {
                    ...
                    if (funcName === 'rgb' || funcName === 'rgba') {
                        colorsInformation.push(new vscode_1.ColorInformation(new vscode_1.Range(new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.name, text, color.lineno - 1)), new vscode_1.Position(color.lineno - 1, exports.getRealCallColumn(color.name, text, color.lineno - 1))), 
                        // @ts-ignore
                        new vscode_1.Color(colors_1.getNumericValue(colorValues[0], 255.0), colors_1.getNumericValue(colorValues[1], 255.0), colors_1.getNumericValue(colorValues[2], 255.0), alpha)));
                    }
                    else if (funcName === 'hsl' || funcName === 'hsla') {
                        const h = colors_1.getAngle(colorValues[0]);
                        const s = colors_1.getNumericValue(colorValues[1], 100.0);
                        const l = colors_1.getNumericValue(colorValues[2], 100.0);
                        const colorRes = colors_1.colorFromHSL(h, s, l, alpha);
                        colorsInformation.push(new vscode_1.ColorInformation(new vscode_1.Range(new vscode_1.Position(color.lineno - 1, exports.getRealColumn(color.name, text, color.lineno - 1)), new vscode_1.Position(color.lineno - 1, exports.getRealCallColumn(color.name, text, color.lineno - 1))), new vscode_1.Color(colorRes.red, colorRes.green, colorRes.blue, colorRes.alpha)));
                    }
                }
            }
            catch (_) {
...
class StylusColorProvider {
    provideDocumentColors(document, token) {
        ...
        const ast = parser_1.flattenAndFilterAst(parser_1.buildAst(documentTxt));
    ...
    provideColorPresentations(color, context, token) {
        ...
        if (color.alpha === 1) {
            label = `#${colors_1.toTwoDigitHex(red256)}${colors_1.toTwoDigitHex(green256)}${colors_1.toTwoDigitHex(blue256)}`;
        }
        else {
            label = `#${colors_1.toTwoDigitHex(red256)}${colors_1.toTwoDigitHex(green256)}${colors_1.toTwoDigitHex(blue256)}${colors_1.toTwoDigitHex(Math.round(color.alpha * 255))}`;
        }
        result.push({ label: label, textEdit: vscode_1.TextEdit.replace(context.range, label) });
        const hsl = colors_1.hslFromColor(color);
        if (hsl.a === 1) {

...
function getDocumentRangePrettyfier(strict = false, sizeLimitCheker = null, loggers = null) {
    loggers = loggers || getLoggers();
    sizeLimitCheker = sizeLimitCheker || getSizeLimitChecker(loggers);
    return new tableDocumentRangePrettyfier_1.TableDocumentRangePrettyfier(getSingleTablePrettyfier(loggers, sizeLimitCheker));
}
...
function getDocumentPrettyfier(strict = true) {
    ...
    return new tableDocumentPrettyfier_1.TableDocumentPrettyfier(new multiTablePrettyfier_1.MultiTablePrettyfier(new tableFinder_1.TableFinder(new tableValidator_1.TableValidator(new selectionInterpreter_1.SelectionInterpreter(strict))), getSingleTablePrettyfier(loggers, sizeLimitCheker), sizeLimitCheker));
}
exports.getDocumentPrettyfier = getDocumentPrettyfier;
function getDocumentPrettyfierCommand() {
    const loggers = getLoggers();
    const sizeLimitCheker = getSizeLimitChecker(loggers);
    return new tableDocumentPrettyfierCommand_1.TableDocumentPrettyfierCommand(new multiTablePrettyfier_1.MultiTablePrettyfier(new tableFinder_1.TableFinder(new tableValidator_1.TableValidator(new selectionInterpreter_1.SelectionInterpreter(true))), getSingleTablePrettyfier(loggers, sizeLimitCheker), sizeLimitCheker));
}

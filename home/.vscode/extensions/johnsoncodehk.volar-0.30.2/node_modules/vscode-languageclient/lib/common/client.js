        ...
        const provider = {
            provideCompletionItems: (document, position, token, context) => {
                ...
                const provideCompletionItems = (document, position, context, token) => {
                    return client.sendRequest(vscode_languageserver_protocol_1.CompletionRequest.type, client.code2ProtocolConverter.asCompletionParams(document, position, context), token).then((result) => client.protocol2CodeConverter.asCompletionResult(result, defaultCommitCharacters), (error) => {
            ...
            resolveWorkspaceSymbol: options.resolveProvider === true
                ? (item, token) => {
                    ...
                    const resolveWorkspaceSymbol = (item, token) => {
                        return client.sendRequest(vscode_languageserver_protocol_1.WorkspaceSymbolResolveRequest.type, client.code2ProtocolConverter.asWorkspaceSymbol(item), token).then(client.protocol2CodeConverter.asSymbolInformation, (error) => {

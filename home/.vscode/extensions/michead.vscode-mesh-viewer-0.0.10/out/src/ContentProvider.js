...
class ContentProvider {
    static imageTemplate(id, data, ext) {
        return `<image style="display:none;" id="${id}" src="data:image/${ext};base64, ${data}"></image>`;
    }
    static htmlImageElements(materials, baseDir) {
        const imageElements = [];
    ...
    static parseMaterial(materials, mtl) {
        ...
        while (i < mtl.length && !/^newmtl/.test(mtl[i].trim())) {
            ...
            if (tokens.length > 1) {
                switch (tokens[0]) {
                    ...
                    case 'Kd':
                    case 'Ks':
                        material = Object.assign({}, material, { [tokens[0]]: [
                                parseFloat(tokens[1]),
                                parseFloat(tokens[2]),
                                parseFloat(tokens[3])
                            ] });
                    ...
                    case 'Ns':
                    case 'd':
                        material = Object.assign({}, material, { [tokens[0]]: parseFloat(tokens[1]) });
                        break;
                    case 'Tr':
                        material = Object.assign({}, material, { d: 1 - parseFloat(tokens[1]) });
                        break;
                    case 'map_Ka':
                    case 'map_Kd':
                    case 'map_Ks':
                    case 'map_Ns':
                    case 'map_d':
                        material = Object.assign({}, material, { [tokens[0]]: tokens[1] });
                        break;
                    default:
...
ContentProvider.DEFAULT_MATERIAL = {
    __default__: {
        Ka: [0, 0, 0],
        Kd: [0.5, 0.5, 0.5],

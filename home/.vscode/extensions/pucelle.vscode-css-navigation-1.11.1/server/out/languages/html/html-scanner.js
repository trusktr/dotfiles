...
class HTMLScanner extends text_scanner_1.TextScanner {
    ...
    scanForSelector() {
        ...
        // 	 class="a b"
        // >
        match = this.match(/<[\w-]+\s*(.*?)>/g, /(?<type>id|class)\s*=\s*['"](.*?)['"]/g, /([\w-]+)/g);
        if (match) {
    ...
    async scanForImportPath() {
        ...
        if (match) {
            ...
            if (tag === 'link' && linkStyleRE.test(match.text)) {
                importPath = utils_1.firstMatch(match.text, hrefRE);
            }
            if (tag === 'style') {
                importPath = utils_1.firstMatch(match.text, srcRE);
            }
        }
        if (importPath) {
            return await file_1.resolveImportPath(path.dirname(vscode_uri_1.URI.parse(this.document.uri).fsPath), importPath);
        }
        return null;
    }
}
